...
class TrainCatan:
    ...
    init_training_environment(activation_function,loss_function,optimizer_function):
        Initialize the reinforcement learning environment
    init_online_plot(title='Figure',plot_counter = 0,make_new_figure = True):
        Initialize the online plot
    calc_averaged_statistics(victories,epsilons,cards,win_rate,learning_rate,n_game_average):
        Calculate statistics which are plotted
    plot_statistics_online(victories,epsilons,cards,win_rate,learning_rate,n_game_average):
        Plots the statistics after each n_game_average games
    learning_rate_decay(episode):
        Learning rate function which starts decaying at a certain amount of games with a certain exponential factor.
    init_epsilon_function():
        Initializes the sigmoid epsilon function.
    ...
    def init_online_plot(self,title='Figure',plot_counter = 0,make_new_figure = True):
        ...
        :param plot_counter:
            number of the plot
        :param make_new_figure:
            whether a new figure shall be created
        """
        if make_new_figure:
    ...
    def calc_averaged_statistics(self,victories,epsilons,cards,win_rate,learning_rate,n_game_average):
        ...
        :param victories:
        	list of player numbers which won a game
        :param epsilons:
            list of epsilons per game
        :param cards:
            list of cards per player and game
        :param win_rate:
            list of win rate per game
        :param learning_rate:
            list of learning_rates per game
        :param n_game_average:
            number of games which shall be averaged
        """
        start_ind = 0
        end_ind = 0
        avg_vic = []
